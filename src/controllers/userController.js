...
const login = async (req, res, next) => {
    ...
    try {
        ...
        if(!user) {
            next({statusCode: 401                                                                                                                     , message: 'Authentication failed. User not found'});
        }
        const isMatch = await user.comparePassword(password);
        if (!isMatch) {
            next({statusCode: 401, message: 'Authentication failed. Wrong password'});
        }
        jwt.sign({_id: user._id, email: user.email, role: user.role}, SECRET_KEY, {algorithm: 'HS256'}, (err, data) => {
            if(err){
                console.log(err);
